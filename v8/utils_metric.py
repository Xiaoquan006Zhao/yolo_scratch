...
def calculate_precision_recall(predictions, targets, scaled_anchor, s):
    ...
    decoded_predictions_nms = torch.tensor(nms(decoded_predictions)).view(1, -1, 6)
    num_predictions = torch.sum(decoded_predictions_nms[:, :, 0] > Config.valid_prediction_threshold).item()
    ...
    ious = ciou(decoded_predictions[..., 1:5][pred_obj & tar_obj], decoded_targets[..., 1:5][pred_obj & tar_obj], is_pred=False)
