...
class PAN(nn.Module):
    def __init__(
        ...
        num_classes,
        use_dropblock=True,
        dropblock_params={"block_size": 3, "p": 0.1},
    ):
        """
        ...
        self.conv1 = ConvBNMish(
            channels_list[2], channels_list[1], kernel_size=1, stride=1, padding=0
        )
        self.upsample1 = nn.UpsamplingBilinear2d(scale_factor=2)
        self.CSP1 = CSPBlock(
            channels_list[2],
            channels_list[1],
            bottleNeck_use_residual=False,
            BottleNeck_repeats=3,
        )

        self.conv2 = ConvBNMish(
            channels_list[1], channels_list[0], kernel_size=1, stride=1, padding=0
        )
        ...
        self.downsample1 = ConvBNMish(
            channels_list[0], channels_list[0], kernel_size=3, stride=2, padding=1
        )
        self.CSP3 = CSPBlock(
            channels_list[1],
            channels_list[1],
        ...
        self.downsample2 = ConvBNMish(
            channels_list[1], channels_list[1], kernel_size=3, stride=2, padding=1
        )
        self.CSP4 = CSPBlock(
            channels_list[2],
            channels_list[2],
    ...
    def forward(self, features):
        ...
        return outputs[
            ::-1
        ]  # predictions returned from more channels to less channels (big to small)
