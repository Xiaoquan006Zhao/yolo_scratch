...
def ciou(box1, box2, IoU_mode=False, WidthHeight_model=False): 
	if not WidthHeight_model: 
		...
		alpha = stable_divide(v, 1 - iou_score + v).unsqueeze(1)


		print(center_distance.shape)
		print(c_diag.shape)
		print(v.shape)
		print(alpha.shape)


		ciou_score = iou_score - stable_divide(center_distance, c_diag) - alpha * v.unsqueeze(1)
